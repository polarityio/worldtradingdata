...
function doLookup(entities, options, cb) {
    ...
    entities.forEach(entity => {
      if (entity.value) {
        ...
        let requestOptions = {
          method: "GET",
          uri: options.url + "/api/v1/stock?symbol=" + entity.value.split(':')[1] + "&api_token=" + options.apiKey,
          json: true
        };
    ...
    async.parallelLimit(tasks, 10, (err, results) => {
        ...
        results.forEach(result => {
          Logger.trace({result: result}, "checking results");
            if(result.body === null || _isMiss(result.body) || result.body.Message === "Error! The requested stock(s) could not be found."){
